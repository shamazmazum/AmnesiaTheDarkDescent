...
cLuxProp_Lamp::cLuxProp_Lamp(const tString &asName, int alID, cLuxMap *apMap) : iLuxProp(asName, alID, apMap,
                                                                                         eLuxPropType_Lamp) {
    mbLit = true;
...
bool cLuxProp_Lamp::CanInteract(iPhysicsBody *apBody) {
    if (CanBeIgnitByPlayer() && !mbLit) return true;
...
eLuxFocusCrosshair cLuxProp_Lamp::GetFocusCrosshair(iPhysicsBody *apBody, const cVector3f &avPos) {
    ...
    if (mCustomFocusCrossHair != eLuxFocusCrosshair_Default && mbLit) {
...
void cLuxProp_Lamp::SetupLampLightConnection() {
    if (msConnectionLight != "") {
        ...
        if (pConnectionLight) {
            mpMap->AddLampLightConnection(this, pConnectionLight, mfConnectionLightAmount, mbConnectionLightUseOnColor,
                                          mbConnectionLightUseSpec);
        } else {
            Error("Light '%s' that is connected to lamp '%s' does not exist!\n", msConnectionLight.c_str(),
                  msName.c_str());
        }
    }
...
bool cLuxProp_Lamp::CanBeIgnitByPlayer() {
    if (mbLit || !mbCanBeLitByPlayer) return false;
    return true;
}
