...
cLuxEnemy_Grunt::cLuxEnemy_Grunt(const tString &asName, int alID, cLuxMap *apMap) : iLuxEnemy(asName, alID, apMap,
                                                                                              eLuxEnemyType_Grunt) {
    mfWaitTime = 0;
...
bool cLuxEnemy_Grunt::StateEventImplement(int alState, eLuxEnemyStateEvent aEvent, cLuxStateMessage *apMessage) {
    ...
    kLuxState(eLuxEnemyState_Alert)
        ...
        kLuxOnUpdate
            ...
            if (gpBase->mpPlayer->GetTerror() >= 1 && mbStuckAtDoor)// && mpMap->DoorIsClosed(mlStuckDoorID))
            {
                ...
                if ((gpBase->mpPlayer->GetTerror() >= 1 && (fDistToPlayer > mfAlertToHuntDistance ||
    ...
    kLuxState(eLuxEnemyState_Search)
        ...
        kLuxOnMessage(eLuxEnemyMessage_TimeOut_2)
            ...
            cAINode *pNode = mpPathfinder->GetNodeAtPos(gpBase->mpPlayer->GetCharacterBody()->GetFeetPosition(), 4, 12,
    ...
    kLuxState(eLuxEnemyState_Hunt)
        ...
        kLuxOnMessage(eLuxEnemyMessage_TimeOut_2)
            ...
            if (CanSeePlayer() && fDist > mfNormalAttackDistance && fDist < mfNormalAttackDistance * 2 &&
...
void cLuxEnemy_Grunt::PatrolEndOfPath() {
    if (IsAtLastPatrolNode()) {
        if (!mbIsSeenByPlayer && DistToPlayer() > 10 && mbAutoRemoveAtPathEnd) {
            SetActive(false);
