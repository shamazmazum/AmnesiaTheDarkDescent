...
void iLuxEnemyLoader::BeforeLoad(cXmlElement *apRootElem, const cMatrixf &a_mtxTransform, cWorld *apWorld,
...
void iLuxEnemyLoader::AfterLoad(cXmlElement *apRootElem, const cMatrixf &a_mtxTransform, cWorld *apWorld,
                                cResourceVarsObject *apInstanceVars) {
    ...
    pEnemy->m_mtxCharMeshOffset = cMath::MatrixRotate(cMath::Vector3ToRad(GetVarVector3f("Body_OffsetRot", 0)),
                                                      eEulerRotationOrder_XYZ);
    pEnemy->m_mtxCharMeshOffset.SetTranslation(
            GetVarVector3f("Body_OffsetTrans", 0) - cVector3f(0, pCharBody->GetSize().y / 2, 0));
    ...
    pEnemy->mfDefaultForwardSpeed[eLuxEnemyPoseType_Biped][eLuxEnemyMoveSpeed_Walk] = GetVarFloat("Walk_ForwardSpeed",
                                                                                                  0);
    pEnemy->mfDefaultBackwardSpeed[eLuxEnemyPoseType_Biped][eLuxEnemyMoveSpeed_Walk] = GetVarFloat("Walk_BackwardSpeed",
                                                                                                   0);
    pEnemy->mfDefaultForwardAcc[eLuxEnemyPoseType_Biped][eLuxEnemyMoveSpeed_Walk] = GetVarFloat("Walk_ForwardAcc", 0);
    pEnemy->mfDefaultForwardDeacc[eLuxEnemyPoseType_Biped][eLuxEnemyMoveSpeed_Walk] = GetVarFloat("Walk_ForwardDeacc",
                                                                                                  0);

    pEnemy->mfDefaultForwardSpeed[eLuxEnemyPoseType_Biped][eLuxEnemyMoveSpeed_Run] = GetVarFloat("Run_ForwardSpeed", 0);
    pEnemy->mfDefaultBackwardSpeed[eLuxEnemyPoseType_Biped][eLuxEnemyMoveSpeed_Run] = GetVarFloat("Run_BackwardSpeed",
                                                                                                  0);
...
void iLuxEnemyLoader::LoadAttackSizeData(const tString &asPrefix, cEnemyAttackSizeData *apData, iLuxEnemy *apEnemy,
                                         cWorld *apWorld) {
...
iLuxEnemy::iLuxEnemy(const tString &asName, int alID, cLuxMap *apMap, eLuxEnemyType aEnemyType) : iLuxEntity(asName,
                                                                                                             alID,
                                                                                                             apMap,
...
void iLuxEnemy::OnRenderSolid(cRendererCallbackFunctions *apFunctions) {
    ...
    if (pPlayer->GetHelperLightLevel()->GetNormalLightLevel() < mfPlayerInDarknessLightLevel &&
        pPlayer->GetAvgSpeed() < 0.05f)
...
void iLuxEnemy::SendMessage(eLuxEnemyMessage aType, float afTime, bool abLocalScope, const cVector3f &avX, float afX,
                            int alX) {
    if (mbDisabled) return;
    if (TriggersDisabled() &&
...
float iLuxEnemy::DrawDebug(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1), _W("Name: '%ls'"),
                    cString::To16Char(msName).c_str());
    afStartY += 14;
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1),
                    ...
                    gsLuxEnemyStates[mCurrentState].c_str(), mbCanSeePlayer ? 1 : 0, mbPlayerDetected ? 1 : 0,
                    mbPlayerInRange ? 1 : 0, mbStuckAtDoor ? 1 : 0,
                    mfFOVMul);
    afStartY += 14;
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1),
                    ...
                    mfHealth, mfFOVMul, cMath::Vector3Dist(mpCharBody->GetFeetPosition(), mvStartPosition),
                    mpMover->GetStuckCounter(), mpMover->GetMoveSpeed(), mpMover->GetWantedSpeedAmount());
    afStartY += 14;

    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1),
                    _W("  DistToPlayer: %f PlayerDirAmount: %f CurrentPatrolNode: %d MoverSpeedMul: %f"),
                    DistToPlayer(), GetPlayerMovementTowardEnemyAmount(),
...
void iLuxEnemy::UpdateCanSeePlayer(float afTimeStep) {
    ...
    if (!bLanternOn && pPlayer->GetHelperLightLevel()->GetNormalLightLevel() < mfPlayerInDarknessLightLevel &&
        pPlayer->GetAvgSpeed() < 0.05f &&
        !mbSkipVisibilityRangeHandicaps)
        fMaxRange = mfDarknessSightRange;

    if (bCrouching && !bLanternOn && !mbSkipVisibilityRangeHandicaps)
        fMaxRange = fMaxRange * mfCrouchVisibleRangeMul;
...
void iLuxEnemy::UpdatePlayerDetected(float afTimeStep) {
    ...
    if (bDetected && !mbPlayerDetected) {
        SendMessage(eLuxEnemyMessage_PlayerDetected, 0, false);
    } else if (!bDetected && mbPlayerDetected) {
...
void iLuxEnemy::UpdatePlayerInRange(float afTimeStep) {
    ...
    if (fDistance <= mfActivationDistance && !mbPlayerInRange) {
...
void iLuxEnemy::UpdateDarknessGlow(float afTimeStep) {
        ...
        if (pCam->GetFrustum()->CollideBoundingVolume(mpMeshEntity->GetBoundingVolume()) != eCollision_Outside) {
            ...
            if (fDistSqrt < mfEnemyDarknessGlowMaxDistance * mfEnemyDarknessGlowMaxDistance) {
                ...
                if (fDist > mfEnemyDarknessGlowMaxDistance * 0.5f) {
                    fDistMul = 1 - ((fDist - mfEnemyDarknessGlowMaxDistance * 0.5f) /
                                    (mfEnemyDarknessGlowMaxDistance * 0.5f));
    ...
    mfDarknessGlowAlphaGoal += gpBase->mpMapHelper->GetLightLevelAtPos(
            mpCharBody->GetFeetPosition() + cVector3f(0, 0.1f, 0));
...
void iLuxEnemy::UpdateHallucination(float afTimeStep) {
    if (!mbHallucination || mfHealth <= 0) return;

    float fDistSqr = cMath::Vector3DistSqr(mpCharBody->GetPosition(),
                                           gpBase->mpPlayer->GetCharacterBody()->GetPosition());
...
void iLuxEnemy::UpdateAlignEntityWithGroundRay(float afTimeStep) {
    if (!mbAlignEntityWithGroundRay) return;
    ...
    cVector3f vStartPos =
            mpCharBody->GetFeetPosition() + cVector3f(0, fStartAdd, 0) + vMoveDir * mpCharBody->GetSize().x * 0.5f;

    bool bIntersect = gpBase->mpMapHelper->GetClosestCharCollider(vStartPos, cVector3f(0, -1, 0), 0.5f, &fDist,
                                                                  &vNormal, NULL);
...
bool
iLuxEnemy::Attack(const cEnemyAttackSizeData &aSizeData, const cEnemyAttackDamageData &aDamageData, float afDamageMul) {
    bool bHitPlayer = false;
    bool bHit = gpBase->mpMapHelper->ShapeDamage(GetAttackShape(aSizeData.mlShapeIdx),
                                                 GetDamageShapeMatrix(aSizeData.mvOffset), mpCharBody->GetPosition(),
                                                 aDamageData.mfMinDamage * afDamageMul,
                                                 aDamageData.mfMaxDamage * afDamageMul,
...
cAINode *
iLuxEnemy::GetSearchForPlayerNode(int alMaxIterations, float afMaxAngleMul, float afMinDistMul, float afMaxDistMul) {
    ...
    for (int i = 0; i < alMaxIterations; ++i) {
        ...
        //The length to use
        float fLength =
                cMath::RandRectf(mfPlayerSearchMinDistMul * afMinDistMul, mfPlayerSearchMaxDistMul * afMaxDistMul) *
                fToPlayerDist;
...
cAINode *
iLuxEnemy::GetPatrolAroundPlayerNode(int alMaxIterations, float afMaxAngleMul, float afMinDistMul, float afMaxDistMul) {
...
bool iLuxEnemy::LineOfSight(const cVector3f &avPos, const cVector3f &avSize, bool abCheckFOV) {
    return LineOfSight(avPos, avSize, abCheckFOV,
                       mpCharBody->GetPosition() + cVector3f(0, mpCharBody->GetSize().y / 2 - 0.1f, 0));
}

bool
iLuxEnemy::LineOfSight(const cVector3f &avPos, const cVector3f &avSize, bool abCheckFOV, const cVector3f &avSourcePos) {
    /////////////////////////////////////////
...
bool iLuxEnemy::InFOV(const cVector3f &avPos) {
    ...
    if (fFOV < k2Pif) {
        cVector3f vStartCenter =
                mpCharBody->GetPosition() + cVector3f(0, mpCharBody->GetSize().y / 2 - 0.2f, 0); //Use eye pos
