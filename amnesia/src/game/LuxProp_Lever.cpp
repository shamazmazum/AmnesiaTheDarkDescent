...
cLuxProp_Lever::cLuxProp_Lever(const tString &asName, int alID, cLuxMap *apMap) : iLuxProp(asName, alID, apMap,
                                                                                           eLuxPropType_Lever) {
...
void cLuxProp_Lever::UpdateAutoMove(float afAngle, float afTimeStep) {
    if (IsInteractedWith() || !mbAutoMoveToAngle || mlStuckState != 0) return;
