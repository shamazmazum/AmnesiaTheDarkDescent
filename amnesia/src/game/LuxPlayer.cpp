...
bool cLuxPlayer::CanDrawCrossHair() {
    ...
    if (!mbScriptShowFocusIconAndCrossHair) return false;
    if (!mbShowCrossHair && mState != eLuxPlayerState_UseItem) {
        if (mpEntityInFocus) {
...
void cLuxPlayer::DrawHud(float afFrameTime) {
    ...
    if (msFocusText != _W("")) {
        ...
        for (size_t i = 0; i < vRows.size(); ++i)
            gpBase->mpGameHudSet->DrawFont(vRows[i], mpFocusFont, cVector3f(400, fFocusTextY + i * 24, 1), 22,
                                           cColor(1, mfFocusTextAlpha), eFontAlign_Center);
...
void cLuxPlayer::CreateCharacterBody(iPhysicsWorld *apPhysicsWorld) {
    ...
    mpCharBody->SetMaxNoSlideSlopeAngle(
            cMath::ToRad(gpBase->mpGameCfg->GetFloat("Player_Body", "MaxNoSlideSlopeAngle", 0)));
