...
void cLuxCredits::Setup(const tString &asMusic, bool abLoopMusic, const tString &asTextCat, const tString &asTextEntry,
                        int alEndNum) {
    msMusic = asMusic;
...
void cLuxCredits::OnDraw(float afFrameTime) {
    ...
    if (mlState == 0 || mlState == 1) {
        ...
        for (size_t i = 0; i < mvTextRows.size(); ++i) {
            ...
            if (mvTextRows[i].size() <= 1) {
                ...
                if (fY >= -fRowAdd[lSize]) {
                    ...
                    //Normal
                    if (lSize == 0)
                        mpGuiSet->DrawFont(mvTextRows[i], mpFontNormal, cVector3f(400, fY, 10), mvNormalFontSize,
                                           cColor(1, fAlpha), eFontAlign_Center);
                        //Header
                    else
                        mpGuiSet->DrawFont(cString::SubW(mvTextRows[i], 1), mpFontHeader, cVector3f(400, fY, 10),
                                           mvHeaderFontSize, cColor(0.8f, fAlpha), eFontAlign_Center);
                }
            }
        ...
        mpGuiSet->DrawFont(kTranslate("General", "TheEnd"), mpFontHeader, cVector3f(400, 290, 10), mvTheEndFontSize,
                           cColor(1, 1), eFontAlign_Center);

        //Secret code
        if (mlEndNum >= 0 && mlEndNum <= 2 && !gpBase->mbPTestActivated) {
            tWString sCode = _W("");
