...
namespace hpl {
    inline void
    SetupBaseDirs(tWStringVec &vDirs, const tWString &asRelativeParent = _W(""), const tWString &asMainFolder = _W(""),
        ...
        vDirs.push_back(PERSONAL_RELATIVEROOT
        PERSONAL_RELATIVEGAME_PARENT);
#ifndef HPL_MINIMAL
        iFileBrowser::msGameDir = cPlatform::GetWorkingDir();
        iFileBrowser::msPersonalDir = PERSONAL_RELATIVEROOT
        PERSONAL_RELATIVEGAME_PARENT;
#endif
        if (userDir) {
            vDirs.push_back(PERSONAL_RELATIVEROOT
