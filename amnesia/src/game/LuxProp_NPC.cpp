...
void cLuxPropLoader_NPC::LoadVariables(iLuxProp *apProp, cXmlElement *apRootElem) {
    ...
    if (mpMesh->GetSkeleton() == NULL) {
        ...
        for (size_t i = 0; i < vBoneNameVec.size(); ++i) {
            ...
            boneData.mvRotVec = cMath::Vector3Normalize(cMath::MatrixMul(mtxInvWorldBone, cVector3f(0, 1,
                                                                                                    0))); //Make sure to normalize since matrix might have scale!
        }
    }
...
cLuxProp_NPC::cLuxProp_NPC(const tString &asName, int alID, cLuxMap *apMap) : iLuxProp(asName, alID, apMap,
                                                                                       eLuxPropType_NPC) {
    mfHeadAngle = 0;
    mfInAreaCount = 0;
