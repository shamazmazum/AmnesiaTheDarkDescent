...
cLuxProp_Item::cLuxProp_Item(const tString &asName, int alID, cLuxMap *apMap) : iLuxProp(asName, alID, apMap,
                                                                                         eLuxPropType_Item) {
    mfAmount = 1.0f;
...
bool cLuxProp_Item::OnInteract(iPhysicsBody *apBody, const cVector3f &avPos) {
    ...
    gpBase->mpInventory->AddItem(msName, mItemType, msSubItemTypeName, msImageFile, mfAmount, msVal, msExtraVal,
                                 &bRemoveProp);
    ...
    if (bRemoveProp) {
        ...
        if (pType->ShowPickUpMessage()) {
            ...
            gpBase->mpMessageHandler->SetMessage(
                    kTranslate("Inventory", "PickedUp") + _W(" ") + kTranslate("Inventory", sEntry), 0);
        }
