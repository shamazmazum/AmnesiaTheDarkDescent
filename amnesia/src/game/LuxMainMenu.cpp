...
cLuxMainMenu::cLuxMainMenu() : iLuxUpdateable("LuxDebugHandler") {
    ...
    mvWindows[eLuxMainMenuWindow_CustomStoryList] = hplNew(cLuxMainMenu_CustomStoryList,
                                                           (mpGuiSet, mpGuiSkin, pCustomStoryWindow));
    ...
    mvTopMenuFontSize.x *=
            (mvScreenSize.y / mvScreenSize.x) / (3.0f / 4.0f);//Make font more narrow to compensate for wide screen.
...
void cLuxMainMenu::OnLeaveContainer(const tString &asNewContainer) {
    //Unlock input if not in window
    if (!gpBase->mpDebugHandler->GetDebugWindowActive()) {
        if (!gpBase->mpConfigHandler->mbFullscreen) {
...
void cLuxMainMenu::CreateTopMenuGui() {
    ...
    if (!gpBase->mbPTestActivated &&
        !gpBase->mpMapHandler->MapIsLoaded() && gpBase->mpSaveHandler->SaveFileExists()) {
    ...
    //Load game
    if (!gpBase->mbPTestActivated &&
        gpBase->mpSaveHandler->SaveFileExists() && !gpBase->mpMapHandler->MapIsLoaded()) {
    ...
    if (!gpBase->mbPTestActivated &&
        gpBase->mbHardMode &&
        gpBase->mpMapHandler->MapIsLoaded()) {
        ////////////////////////////
...
#else
            tWString sNumTinderboxes = tWString(
                    std::to_wstring(static_cast<long long>(gpBase->mpPlayer->GetTinderboxes())));
#endif
        ...
        mpNumTinderboxes->SetAutogenerateSize(true);
        mpNumTinderboxes->SetPosition(
                mpNumTinderboxes->GetLocalPosition() - cVector3f(mpNumTinderboxes->GetSize().x * 0.5f, 0, 0));
    ...
    ///////////////
    //Exit
    if (!gpBase->mpMapHandler->MapIsLoaded() || gpBase->mbExitMenuDirectly) {
        ...
        if (!gpBase->mbPTestActivated && !gpBase->mpPlayer->IsDead() && !gpBase->mbHardMode) {
...
void cLuxMainMenu::CreateBackground() {
    ...
    if (gpBase->mpMapHandler->MapIsLoaded()) {
        ...
        mpLogoGfx = mpGui->CreateGfxImage(gpBase->mpMenuCfg->GetString("Main", "MenuLogo", "menu_logo.tga"),
                                          eGuiMaterial_Alpha);
        if (mpLogoGfx) {
...
void cLuxMainMenu::RenderBlurTexture() {
    ...
    pTempBlurTexture->CreateFromRawData(cVector3l((int) mvScreenSize.x, (int) mvScreenSize.y, 0), ePixelFormat_RGBA,
                                        NULL);
...
bool cLuxMainMenu::PressContinue(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(_W(""), kTranslate("MainMenu", "Continue old game?"),
                                                                  kTranslate("MainMenu", "Yes"),
                                                                  kTranslate("MainMenu", "No"),
                                                                  this,
...
bool cLuxMainMenu::PressStartGame(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (gpBase->mbAllowHardmode && gpBase->mpCustomStory == 0) {
        ...
        mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
        cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(_W(""),
                                                                      kTranslate("MainMenu", "Start a new game?"),
                                                                      kTranslate("MainMenu", "Yes"),
                                                                      kTranslate("MainMenu", "No"),
                                                                      this,
...
bool cLuxMainMenu::PressExitToMainMenu(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
    cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(_W(""), kTranslate("MainMenu",
                                                                                     "Sure you want to exit to main menu?"),
                                                                  kTranslate("MainMenu", "Yes"),
                                                                  kTranslate("MainMenu", "No"),
                                                                  this,
...
bool cLuxMainMenu::PressExitAndSave(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
    cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(_W(""), kTranslate("MainMenu",
                                                                                     "Sure you want to exit and save?"),
                                                                  kTranslate("MainMenu", "Yes"),
                                                                  kTranslate("MainMenu", "No"),
                                                                  this,
