...
void cLuxJournalStateData::OnDraw(float afFrameTime) {
    ...
    if (mState == eLuxJournalState_OpenNote ||
        ...
        mpJournal->mpGuiSet->DrawFont(mpJournal->msHeader, mpJournal->mpFontMenu, vHeaderPos,
                                      mpJournal->mfNoteHeaderFontSize, cColor(1, fAlpha), eFontAlign_Center);
        ...
        for (size_t i = 0; i < pPage->mvRows.size(); ++i) {
            mpJournal->mpGuiSet->DrawFont(pPage->mvRows[i], mpJournal->mpFontDefault, vPos, mpJournal->mvNoteFontSize,
                                          cColor(1, fAlpha));
    ...
    if (mpJournal->mpStateBackgroundGfx && mpJournal->mCurrentState == mState) {
        ...
        mpJournal->mpGuiSet->DrawGfx(mpJournal->mpStateBackgroundGfx,
                                     cVector3f(400 - vSize.x / 2, 300 - vSize.y / 2, 1), -1, cColor(1, fAlpha));
    }
}
...
void cLuxJournal::Exit() {
    ...
    if (mbForceInstantExit) {
        gpBase->mpProgressLogHandler->AddLog(eLuxProgressLogLevel_Low,
...
void cLuxJournal::SetupLabel(cWidgetLabel *apLabel, const cVector2f &avSize, int alIdx, eLuxJournalState aState,
                             iFontData *apFont, eFontAlign aFontAlign) {
...
void
cLuxJournal::SetupNavigationWidgets(eLuxJournalState aState, int alListIndex, int alForwardIndex, int alBackwardIndex,
                                    cWidgetDummy *apRoot) {
    cVector2f vSize(65, 46);
    cVector3f vPos = cVector3f(400 + mfNoteTextWidth / 2 - vSize.x - 80,
                               mfNoteTextStartY + ((float) mlNoteMaxPageRows + 1) * mfNoteRowDist, 3);
    ...
    //Forward
    mpImageForward[aState] = mpGuiSet->CreateWidgetImage("journal/note_forward.tga", vPos, vSize, eGuiMaterial_Alpha,
                                                         false, apRoot);
    ...
    if (aState == eLuxJournalState_Diaries || aState == eLuxJournalState_Notes || aState == eLuxJournalState_QuestLog) {
        cWidgetLabel *pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(350, mfBackTextY, 3), cVector2f(100, 24),
                                                           kTranslate("Journal", "MainBack"), apRoot);
        ...
        cWidgetImage *pImage = mpGuiSet->CreateWidgetImage("journal/note_return.tga", 0, -1, eGuiMaterial_Alpha, false,
                                                           apRoot);
    ...
    mpImageBackward[aState] = mpGuiSet->CreateWidgetImage("journal/note_backward.tga", vPos, vSize, eGuiMaterial_Alpha,
                                                          false, apRoot);
...
void cLuxJournal::CreateMainGui() {
    ...
    //Notes
    pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(300, 200, 3), cVector2f(200, 25), kTranslate("Journal", "Notes"),
                                         pState);
    ...
    //Diaries
    pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(300, 250, 3), cVector2f(200, 25), kTranslate("Journal", "Diaries"),
                                         pState);
    ...
    //Quest Log
    pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(300, 300, 3), cVector2f(200, 25), kTranslate("Journal", "Quest Log"),
                                         pState);
    ...
    //Back
    pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(300, mfBackTextY, 3), cVector2f(200, 25),
                                         kTranslate("Journal", "MainBack"), pState);
...
void cLuxJournal::CreateNotesGui() {
    ...
    /////////////////////////////
    // Header
    pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(250, mfNoteListHeaderY, 3), cVector2f(300, 32),
                                         kTranslate("Journal", "Notes"), pRoot);
    ...
    if (mvNotes.empty()) {
        ...
        for (size_t i = 0; i < mvNotes.size(); ++i) {
            ...
            //Add label
            pLabel = mpGuiSet->CreateWidgetLabel(vStartPos, cVector2f(300, 24),
                                                 kTranslate("Journal", pNote->msNameEntry), pRoot);
            ...
            //Add icon
            pImage = mpGuiSet->CreateWidgetImage(pNote->msIconFile, vStartPos + cVector3f(-25, 2, -0.05f),
                                                 cVector2f(20, 20), eGuiMaterial_Alpha, false, pRoot);
...
void cLuxJournal::CreateDiariesGui() {
    ...
    // Header
    pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(250, mfNoteListHeaderY, 3), cVector2f(300, 32),
                                         kTranslate("Journal", "Diaries"), pRoot);
    ...
    if (mvDiaryContainers.empty()) {
        vStartPos.y = 270;
        pLabel = mpGuiSet->CreateWidgetLabel(vStartPos, cVector2f(300, 24), kTranslate("Journal", "DiariesEmpty"),
                                             pRoot);
        ...
        for (size_t cont = 0; cont < mvDiaryContainers.size(); ++cont) {
            ...
            for (size_t i = 0; i < pCont->mvDiaries.size(); ++i) {
                ...
                pLabel = mpGuiSet->CreateWidgetLabel(vStartPos, cVector2f(300, 24),
                                                     kTranslate("Journal", pDiary->msNameEntry), pRoot);
                ...
                pImage = mpGuiSet->CreateWidgetImage(pDiary->msIconFile, vStartPos - cVector3f(23, 0, 1),
                                                     cVector2f(20, 20), eGuiMaterial_Alpha, false, pRoot);
...
void cLuxJournal::CreateQuestNotesGui() {
    ...
    // Header
    pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(250, mfNoteListHeaderY, 3), cVector2f(300, 32),
                                         kTranslate("Journal", "Quest Log"), pRoot);
    ...
    if (!bHasActiveQuests) {
        vStartPos.y = 270;
        pLabel = mpGuiSet->CreateWidgetLabel(vStartPos, cVector2f(300, 24), kTranslate("Journal", "QuestsEmpty"),
                                             pRoot);
...
void cLuxJournal::CreateOpenNoteGui() {
    ...
    // Diary and note
    for (int i = 0; i < 3; ++i) {
        eLuxJournalState state = i == 0 ? eLuxJournalState_OpenNote : (i == 1 ? eLuxJournalState_OpenDiary
...
bool cLuxJournal::ListTextOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (pData->mfEffectfAlpha > 0) {
        ...
        if (pLabel->GetTextAlign() == eFontAlign_Center) {
            ...
            mpGuiSet->DrawFont(apWidget->GetText(), apWidget->GetDefaultFontType(), vPos, vSize,
                               cColor(1, 0, 0, fAlpha), eFontAlign_Center);
        }
            //////////////////////
            //Left Aligned
            ...
            float fSizeAdd = apWidget->GetDefaultFontType()->GetLength(vSize, apWidget->GetText().c_str()) -
                             apWidget->GetDefaultFontType()->GetLength(apWidget->GetDefaultFontSize(),
                                                                       apWidget->GetText().c_str());

            cVector3f vPos = apWidget->GetGlobalPosition() - cVector3f(fSizeAdd / 2, 2, 1);

            mpGuiSet->DrawFont(apWidget->GetText(), apWidget->GetDefaultFontType(), vPos, vSize,
                               cColor(1, 0, 0, fAlpha), eFontAlign_Left);
        }
...
bool cLuxJournal::UIListenerButtonPress(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (!(lType == eLuxJournalState_OpenNote || lType == eLuxJournalState_OpenDiary || lType == eLuxJournalState_OpenNarratedDiary))
        return false;
