...
void iLuxPropLoader::BeforeLoad(cXmlElement *apRootElem, const cMatrixf &a_mtxTransform, cWorld *apWorld,
...
void iLuxPropLoader::AfterLoad(cXmlElement *apRootElem, const cMatrixf &a_mtxTransform, cWorld *apWorld,
                               cResourceVarsObject *apInstanceVars) {
    ...
    if (pProp->mpMeshEntity && pProp->mpMeshEntity->GetAnimationStateNum() > 0 &&
        GetVarBool("RandomizeAnimationStart", true)) {
    ...
    if (apInstanceVars) {
        if (pProp->mpMeshEntity) {
            pProp->mpMeshEntity->SetRenderFlagBit(eRenderableFlag_ShadowCaster,
                                                  apInstanceVars->GetVarBool("CastShadows", true));
        }
...
iLuxProp::iLuxProp(const tString &asName, int alID, cLuxMap *apMap, eLuxPropType aPropType) : iLuxEntity(asName, alID,
                                                                                                         apMap,
                                                                                                         eLuxEntityType_Prop) {
    mPropType = aPropType;
...
iLuxProp::~iLuxProp() {
    ...
    ////////////////////
    // Attachment
    if (mpAttachmentParent && mpMap && !mpMap->IsDeletingAllWorldEntities()) {
...
void iLuxProp::SetEffectsActive(bool abActive, bool abFadeAndPlaySounds) {
    ...
    if (mbEffectsActive) {
        ...
        ////////////////
        // Effect alpha
        if (!abFadeAndPlaySounds) mfEffectsAlpha = 1.0f;
    }
        ///////////////////////
        // Effects Off stuff
    else {
        ...
        ////////////////
        // Effect alpha
        if (!abFadeAndPlaySounds) mfEffectsAlpha = 0.0f;
    }
...
void iLuxProp::MoveLinearTo(const cVector3f &avGoal, float afAcc, float afMaxSpeed, float afSlowdownDist,
                            bool abResetSpeed) {
    if (!mbMoving && mfMoveStartCount <= 0) {
...
void iLuxProp::MoveAngularTo(const cMatrixf &a_mtxGoal, float afAcc, float afMaxSpeed, float afSlowdownDist,
                             bool abResetSpeed,
...
void iLuxProp::PlayAnimation(const tString &asName, float afFadeTime, bool abLoop, const tString &asCallback) {
    ...
    if (!abLoop) mlCurrentNonLoopAnimIndex = lIdx;
...
void iLuxProp::UpdateAttachedProps(float afTimeStep, bool abForceUpdate) {
    ...
    for (; it != mlstAttachedProps.end(); ++it) {
        ...
        cMatrixf mtxProp = cMath::MatrixMul(GetMainBody()->GetLocalMatrix(),
                                            cMath::MatrixMul(pAttachProp->m_mtxOffset,
                                                             pProp->mvBodyExtraData[0].m_mtxLocalTransform));
...
void iLuxProp::UpdateMoving(float afTimeStep) {
    ...
    if (mbMovingLinear) {
        ...
        float fSpeedSqr = cMath::Vector3DistSqr(m_mtxLastBodyMoveMatrix.GetTranslation(),
                                                GetMainBody()->GetLocalMatrix().GetTranslation());
        if (fSpeedSqr > fMinLinear * fMinLinear) bHasMoveSpeed = true;
    }
    if (!bHasMoveSpeed && mbMovingAngular) {
        const float fMinAngular = 0.001f * (1.0f / 60.0f);

        cVector3f vVel = cMath::MatrixEulerAngleDistance(m_mtxLastBodyMoveMatrix.GetRotation(),
                                                         GetMainBody()->GetLocalMatrix().GetRotation());
    ...
    if (bHasMoveSpeed && mfMovingVolume < 1) {
        ...
        // Increase volume
        mfMovingVolume +=
                afTimeStep * 1.2f;//Important that sounds fades in faster than out! (in case mbMoving "flickers" )
        ...
        //////////////////////////////
        //Fade out move sound
    else if (!bHasMoveSpeed && mfMovingVolume > 0) {
        mfMovingVolume -= afTimeStep;
        if (mfMovingVolume < 0) {
            mfMovingVolume = 0;
    ...
    ///////////////////////////
    // Stop movement
    if (!mbMovingLinear && !mbMovingAngular) {
        StopMove();
    }
}
...
void iLuxProp::UpdateAngularMoving(float afTimeStep) {
    ...
    if (mbMoveAngularUseOffset) {
        cMatrixf mtxNewBody = cMath::MatrixMul(cMath::MatrixRotate(vMoveVel * afTimeStep, eEulerRotationOrder_XYZ),
                                               pBody->GetLocalMatrix().GetRotation());
...
void iLuxProp::UpdateCheckIfOutsidePlayer(float afTimeStep) {
    if (!mbCheckOutsidePlayer) return;
