...
void cLuxDemoEnd::OnEnterContainer(const tString &asOldContainer) {
    ...
    // Load background
    iTexture *pTex = gpBase->mpEngine->GetResources()->GetTextureManager()->Create2D(msBackgroundFile, false,
                                                                                     eTextureType_Rect);
...
void cLuxDemoEnd::OnDraw(float afFrameTime) {
    ...
    if (mpGfxBackground) {
        ...
        mpGuiSet->DrawGfx(mpGfxBackground, cVector3f(400 * (1 - fRatioMul), 0, 5), cVector2f(800 * fRatioMul, 600),
                          cColor(1, 1));
    }
    ...
    if (mpFontMessage) {
        ...
        for (size_t i = 0; i < mvTextRows.size(); ++i) {
            ...
            if (mvTextRows[i].empty() == false) {
                mpGuiSet->DrawFont(mvTextRows[i], mpFontMessage, cVector3f(400.0f - mfTextWidth / 2.0f, fY, 10),
                                   mvMessageFontSize, mMessageFontColor, eFontAlign_Left);
            }

            fY += mvMessageFontSize.y + 1;
        }
        mpGuiSet->DrawFont(kTranslate("Demo", "AvailableAt"), mpFontMessage, cVector3f(400.0f, mfAvailableAtY, 10),
                           mvAvailableAtFontSize, mAvailableAtFontColor, eFontAlign_Center);
    }
