...
void cLuxHintHandler::DrawHintText(float afFrameTime, cGuiSet *apGuiSet) {
    ...
    if (!mbActive || mfAlpha <= 0) return;

    float fX = mHintOscill.val * 0.5;
    cColor hintCol(1, fX, fX, mfAlpha);
    apGuiSet->DrawFont(mpFont, cVector3f(400, mfYPos, 20), mvFontSize, hintCol, eFontAlign_Center,
    ...
    if (vRows.size() == 1) {
        apGuiSet->DrawFont(mpFont, cVector3f(400, fStartY, 20), mvFontSize, cColor(1, mfAlpha), eFontAlign_Center,
                           eGuiMaterial_FontNormal,
                           _W(" %ls"), msCurrentText.c_str());
    } else {
        for (size_t i = 0; i < vRows.size(); ++i) {
            apGuiSet->DrawFont(mpFont,
                               cVector3f(400 - fMaxWidth * 0.5f, fStartY + (float) i * (mvFontSize.y + 2.0f), 20),
                               mvFontSize, cColor(1, mfAlpha), eFontAlign_Left, eGuiMaterial_FontNormal,
                               _W(" %ls"), vRows[i].c_str());
        }
    }
...
bool cLuxHintHandler::Add(const tString &asName, const tWString &asMessage, float afTimeShown) {
    ...
    mfShowTextCount =
            afTimeShown <= 0 ? gpBase->mpHelpFuncs->GetStringDuration(msCurrentText) * mfTextDurationMul : afTimeShown;
    mfAlpha = 0;
