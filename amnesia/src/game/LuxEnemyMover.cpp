...
void cLuxEnemyMover::UpdateMoveAnimation(float afTimeStep) {
    ...
    if (prevMoveState != mMoveState) {
        ...
        if (mMoveState == eLuxEnemyMoveState_Backward) {
            //Log(" To Backward\n");
            mpEnemy->PlayAnim(mpEnemy->GetBackwardAnimationName(), true, 0.4f, true, mpEnemy->mfMoveSpeedAnimMul, false,
                              false);
        }
            //Stopped
            ...
            mpEnemy->PlayAnim(mpEnemy->GetWalkAnimationName(), true, 0.5f, true, mpEnemy->mfMoveSpeedAnimMul, bSync,
                              false);
        }
            //Running
        else if (mMoveState == eLuxEnemyMoveState_Running) {
            bool bSync = prevMoveState == eLuxEnemyMoveState_Walking;
            //Log(" To Run. Synch: %d\n", bSync);

            mpEnemy->PlayAnim(mpEnemy->GetRunAnimationName(), true, 0.5f, true, mpEnemy->mfMoveSpeedAnimMul, bSync,
                              false);
        }
    }
