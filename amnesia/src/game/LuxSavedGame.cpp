...
void cLuxSavedGameEntity::FromEntity(iLuxEntity *apEntity) {
    ...
    if (apEntity->GetEntityType() == eLuxEntityType_Prop) {
        ...
        for (; attachIt != pProp->mlstAttachedProps.end(); ++attachIt) {
            ...
            Log("Saving attached prop: '%s' '%s' (%s)\n", attachProp.msName.c_str(), attachProp.msFileName.c_str(),
                attachProp.m_mtxOffset.ToString().c_str());
    ...
    ////////////////////////
    //Callbacks
    for (tLuxCollideCallbackListIt it = apEntity->mlstCollideCallbacks.begin();
         it != apEntity->mlstCollideCallbacks.end(); ++it) {
    ...
    // Particle systems
    {
        ...
        while (psIt.HasNext()) {
            ...
            if (pPS->IsSaved() && pPS->GetEntityParent() == NULL && pPS->GetParent() == NULL &&
                pPS->IsDying() == false)// && pPS->GetUniqueID()<0)
            {
        ...
        while (it.HasNext()) {
            ...
            //Full functions
            {
                ...
                if (pEntity->msLookAtCallback != "")
                    mlstEntityFunc_Look.Add(cLuxSavedGameEntityFunc(pEntity->GetID(), pEntity->msLookAtCallback,
                                                                    pEntity->mbLookAtCallbackRemove));

                // Interact
                if (pEntity->msInteractCallback != "")
                    mlstEntityFunc_Interact.Add(cLuxSavedGameEntityFunc(pEntity->GetID(), pEntity->msInteractCallback,
                                                                        pEntity->mbInteractCallbackRemove));

                // Callback
                if (pEntity->msCallbackFunc != "")
                    mlstEntityFunc_Callback.Add(
                            cLuxSavedGameEntityFunc(pEntity->GetID(), pEntity->msCallbackFunc, false));

                // State Change
                if (pEntity->msConnectionStateChangeCallback != "")
                    mlstEntityFunc_StateChange.Add(
                            cLuxSavedGameEntityFunc(pEntity->GetID(), pEntity->msConnectionStateChangeCallback, false));
            }
            ...
            if (entityType == eLuxEntityType_Enemy) {
                ...
                if (!bAddedToColliderAndConnectEntities &&
...
void cLuxSavedGameMap::ToMap(cLuxMap *apMap) {
    ...
    tLuxEntityList lstSetupEntities;
    {
        ...
        while (entIt.HasNext()) {
            iLuxEntity *pEntity = entIt.Next();
            if (pEntity->GetFullGameSave() && !EntitySaveDataExists(pEntity->GetID())) {
            ...
            if (entityType == eLuxEntityType_Prop) {
                ...
                if (propType == eLuxPropType_SwingDoor) {
                    ...
                    if (pObject->GetContainedItem() != "") {
                        ...
                        if (ExistsInSet(lID, setUnbrokenItemContainers) == false &&
                            ExistsInSet(lID, setBrokenContainersWithActiveItem) == false) {
