...
cLuxProp_Chest::cLuxProp_Chest(const tString &asName, int alID, cLuxMap *apMap) : iLuxProp(asName, alID, apMap,
                                                                                           eLuxPropType_Chest) {
    mbLocked = false;
...
bool cLuxProp_Chest::OnInteract(iPhysicsBody *apBody, const cVector3f &avPos) {
    ...
    tWString sText = kTranslate("Game", "InteractChest01") + _W(" ") + cString::ToStringW(mlCoinsNeeded) + _W(" ") +
                     kTranslate("Game", "InteractChest02") + _W("\n");
    sText += kTranslate("Game", "InteractChest03") + _W(" ") + cString::ToStringW(gpBase->mpPlayer->GetCoins()) +
             _W("\n");
