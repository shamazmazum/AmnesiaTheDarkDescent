...
cLuxArea_Liquid::cLuxArea_Liquid(const tString &asName, int alID, cLuxMap *apMap) : iLuxArea(asName, alID, apMap,
                                                                                             eLuxAreaType_Liquid) {
...
void cLuxArea_Liquid::OnUpdate(float afTimeStep) {
    ...
    for (size_t i = 0; i < vBodies.size(); ++i) {
        ...
        //Check for collision
        if (bInsideWater) {
            if (pPhysicsWorld->CheckShapeCollision(pBody->GetShape(), pBody->GetLocalMatrix(), pAreaBody->GetShape(),
