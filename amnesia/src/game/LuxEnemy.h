...
public:
    cEnemyAttackDamageData() : mfMinDamage(0), mfMaxDamage(0), mfForce(0), mfMaxImpulse(0), mlStrength(0),
                               mfHitSpeed(3),
...
public:
    ...
    void SendMessage(eLuxEnemyMessage aType, float afTime = 0, bool abLocalScope = false, const cVector3f &avX = 0,
                     float afX = 0, int alX = 0);
    ...
    GetBackwardAnimationName() { return msBackwardAnimationName[eLuxEnemyMoveType_Normal][mCurrentPose]; }

    virtual const tString &
    GetIdleAnimationName() { return msIdleAnimationName[eLuxEnemyMoveType_Normal][mCurrentPose]; }

    virtual const tString &
    GetWalkAnimationName() { return msWalkAnimationName[eLuxEnemyMoveType_Normal][mCurrentPose]; }
...
protected:
    ...
    bool
    Attack(const cEnemyAttackSizeData &aSizeData, const cEnemyAttackDamageData &aDamageData, float afDamageMul = 1.0f);

    void SetMoveSpeed(eLuxEnemyMoveSpeed aType);

    cAINode *GetSearchForPlayerNode(int alMaxIterations = 5, float afMaxAngleMul = 1.0f, float afMinDistMul = 1.0f,
                                    float afMaxDistMul = 1.0f);

    cAINode *GetPatrolAroundPlayerNode(int alMaxIterations = 5, float afMaxAngleMul = 1.0f, float afMinDistMul = 1.0f,
                                       float afMaxDistMul = 1.0f);
