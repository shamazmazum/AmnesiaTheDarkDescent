...
bool cLuxLineOfSightCallback::BeforeIntersect(iPhysicsBody *apBody) {
    if (apBody->IsCharacter() || apBody->GetCollide() == false ||
        (mbCheckShadow && apBody->GetBlocksLight() == false)) {
        return false;
    }
...
bool cLuxClosestEntityCallback::BeforeIntersect(iPhysicsBody *apBody) {
    ...
    iLuxEntity *pEntity = (iLuxEntity *) apBody->GetUserData();
    if (pEntity) {
        if (!pEntity->IsActive()) return false;
...
bool cLuxMapHelper::ShapeDamage(iCollideShape *apShape, const cMatrixf &a_mtxTransform, const cVector3f &avOrigin,
                                ...
                                int alStrength, float afHitSpeed, eLuxDamageType aDamageType,
    ...
    for (size_t i = 0; i < vBodies.size(); ++i) {
        ...
        bool bCollide = pPhysicsWorld->CheckShapeCollision(apShape, a_mtxTransform, pBody->GetShape(),
...
bool cLuxMapHelper::CheckLineOfSight(const cVector3f &avStart, const cVector3f &avEnd, bool abCheckShadows) {
    ...
    return !mLineOfSightCallback.GetIntersected();
}
//-----------------------------------------------------------------------
...
bool cLuxMapHelper::GetClosestCharCollider(const cVector3f &avStart, const cVector3f &avDir, float afRayLength,
...
float
cLuxMapHelper::GetLightLevelAtPos(const cVector3f &avPos, std::vector<iLight *> *apSkipLightsVec, float afRadiusAdd) {
    ////////////////////////////
    //Check so there really is a world
    cLuxMap *pCurrentMap = gpBase->mpMapHandler->GetCurrentMap();
    ...
    GetLightsAtNode(pWorld->GetRenderableContainer(eWorldContainerType_Static)->GetRoot(), lstIntersectingLights,
                    avPos);
    GetLightsAtNode(pWorld->GetRenderableContainer(eWorldContainerType_Dynamic)->GetRoot(), lstIntersectingLights,
                    avPos);
        ...
        for (; it != apNode->GetObjectList()->end(); ++it) {
            ...
            switch (pLight->GetLightType()) {
                ...
                case eLightType_Point:
                    bAdd = cMath::CheckPointInSphereIntersection(avPos, pLight->GetWorldPosition(),
                                                                 pLight->GetRadius());
                    break;
