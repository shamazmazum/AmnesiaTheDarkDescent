...
void cLuxPreMenu::ButtonPressed() {
    if (mpCurrentSection && !mpCurrentSection->mbAllowSkipping) return;
...
void cLuxPreMenu::OnDraw(float afFrameTime) {
    ...
    if (mpCurrentBackground) {
        ...
        mpGuiSet->DrawGfx(mpCurrentBackground, cVector3f(400 * (1 - fRatioMul), 0, 0), cVector2f(800 * fRatioMul, 600),
                          cColor(1, 1));
    }
}
    ...
    switch (mCurrentState) {
        ...
        case eLuxPreMenuState_FastFadeOut: {
            if (mfAlphaFade == 1) {
                ...
                if (!gpBase->mpConfigHandler->mbFirstStart) {
                ...
                if (mlCurrentSectionIdx == (int) mvSections.size()) {
                    ...
                    if (mpCurrentSection) {
                        ...
                        if (mpCurrentSection->msMusic != "") {
                            ...
                            float fFadeSpeed = mpCurrentSection->mfMusicFadeTime == 0 ? 100.0f : 1.0f /
                                                                                                 mpCurrentSection->mfMusicFadeTime;
                            pMusHandler->Play(mpCurrentSection->msMusic, mpCurrentSection->mfMusicVolume, fFadeSpeed,
                                              true, false);
                        }
                    }
                }
        ...
        case eLuxPreMenuState_ShowPremenuSection: {
            if (!mpCurrentSection->HasTextElements() &&
                !mpCurrentSection->HasGammaSettings() &&
                mfTimer <= 0) {
                mCurrentState = eLuxPreMenuState_FadeOut;
            }
        }
