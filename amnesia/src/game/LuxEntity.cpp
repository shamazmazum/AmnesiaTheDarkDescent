...
cSoundEntity *
iLuxEntity::PlaySound(const tString &asName, const tString &asFile, bool abRemoveWhenDone, bool abAttach) {
...
void iLuxEntity::AddConnection(const tString &asName, iLuxEntity *apEntity, bool abInvertStateSent, int alStatesUsed,
                               const tString &asCallbackFunc) {
    cLuxEntityConnection *pConnection = hplNew(cLuxEntityConnection,
                                               (asName, apEntity, abInvertStateSent, alStatesUsed, asCallbackFunc));
...
void iLuxEntity::UpdatePlayerLookAt(float afTimeStep) {
    ...
    for (int i = 0; i < GetBodyNum(); i++) {
        ...
        if (cMath::CheckAABBLineIntersection(pBV->GetMin(), pBV->GetMax(), vStart, vCentreLineEnd, &vIntersection,
                                             NULL) == false) {
            continue;
        }
...
bool iLuxEntity::CollidesWithPlayer() {
    ...
    for (int i = 0; i < GetBodyNum(); ++i) {
        ...
        if (pPhysicsWorld->CheckShapeCollision(pBody->GetShape(), pBody->GetLocalMatrix(), pPlayerBody->GetShape(),
                                               pPlayerBody->GetLocalMatrix(), collideData, 1, false)) {
            return true;
        }
    }
    return false;
