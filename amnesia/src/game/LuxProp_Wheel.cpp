...
cLuxProp_Wheel::cLuxProp_Wheel(const tString &asName, int alID, cLuxMap *apMap) : iLuxProp(asName, alID, apMap,
                                                                                           eLuxPropType_Wheel) {
...
float cLuxProp_Wheel::OnInteractDebugDraw(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 5), 12, cColor(1, 1), _W("WheelAngle: %f, Max: %f, Min: %f"),
                    cMath::ToDeg(mfAngle),
...
void cLuxProp_Wheel::UpdateAutoRotation(float afTimeStep) {
    ...
    if (!mbAutoMoving && mbSlowDownRotation) {
        cVector3f vBodyVel = mpWheelBody->GetAngularVelocity();
