...
void cLuxMap::OnRenderSolid(cRendererCallbackFunctions *apFunctions) {
    if (!gpBase->mpDebugHandler->GetShowEntityInfo()) return;
...
void cLuxMap::SetCheckPoint(const tString &asName, const tString &asStartPos, const tString &asCallback) {
    ...
    for (int i = gpBase->mpMusicHandler->GetMaxPrio(); i >= 0; --i) {
        ...
        if (pMusic->msFile != "" && pMusic->mbLoop && !pMusic->mbSpecialEffect) {
...
void cLuxMap::LoadCheckPoint() {
    gpBase->mpDebugHandler->AddMessage(
            _W("Loading check point ") + cString::To16Char(msCheckPointName) + _W(" Count: ") +
            cString::ToStringW(mlCheckPointCount), false);
    gpBase->mpProgressLogHandler->AddLog(eLuxProgressLogLevel_High,
                                         "Loading checkpoint " + msCheckPointName + " count: " +
                                         cString::ToString(mlCheckPointCount));
    ...
    if (msCheckPointMusic != "")
        gpBase->mpMusicHandler->Play(msCheckPointMusic, true, mfCheckPointMusicVolume, 1, mlCheckPointMusicPrio,
                                     mbCheckPointMusicResume, false);
...
void cLuxMap::CreateEntity(const tString &asName, const tString &asFile, const cMatrixf &a_mtxTransform,
...
iLuxEntity *cLuxMap::GetEntityByName(const tString &asName, eLuxEntityType aType, int alSubType) {
    ...
    if (!LuxIsCorrectType(pEntity, aType, alSubType)) return NULL;
...
int cLuxMap::GetInRangeEnemyNum() {
    ...
    while (it.HasNext()) {
        iLuxEnemy *pEnemy = it.Next();
        if (pEnemy->IsActive() == false) continue;
        if (!pEnemy->GetPlayerInRange()) continue;
...
void cLuxMap::AddCompletionAmount(int alAmount, float afDelay) {
    float fPrevious =
            mlTotalCompletionAmount == 0 ? 0 : (float) mlCurrentCompletionAmount / (float) mlTotalCompletionAmount;
...
cLuxMap::AddLampLightConnection(cLuxProp_Lamp *apLamp, iLight *apLight, float afAmount, bool abUseOnColor,
                                bool abUseSpec) {
...
void cLuxMap::CalculateTotalCompletionAmount() {
    ...
    gpBase->mpDebugHandler->AddMessage(_W("Total completion value: ") + cString::ToStringW(mlTotalCompletionAmount),
                                       false);
}
...
void cLuxMap::UpdateTimers(float afTimeStep) {
    ...
    for (tLuxEventTimerListIt it = mlstTimers.begin(); it != mlstTimers.end();) {
        ...
        if (pTimer->mfCount <= 0 && !pTimer->mbDestroyMe) {
