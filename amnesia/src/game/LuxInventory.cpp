...
cLuxInventory_Item::cLuxInventory_Item(const tString &asName, eLuxItemType aType, const tString &asSubType,
                                       cLuxInventory *apInventory) {
...
bool cLuxInventory_Slot::OnMouseDown(iWidget *apWidget, const cGuiMessageData &aData) {
    if (GetItem()) {
        ...
        cVector3f vItemPos =
                apWidget->GetGlobalPosition() + cVector3f((vSize - GetItem()->GetImage()->GetImageSize())) * 0.5f;
...
bool cLuxInventory_Slot::OnMouseUp(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (pItem && pPickedItem && pPickedItem != pItem) {
        if (!mpInventory->CheckSpecialCombineAction(pItem, pPickedItem, mlIdx)) {
...
bool cLuxInventory_Slot::OnMouseEnter(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (pItem && !mpInventory->mbMessageActive) {
        ///////////////////////////
        //Display combine info
        if (pPickedItem && pPickedItem != pItem) {
            tWString sCombine = kTranslate("Inventory", "Combine") + _W(" ") +
                                kTranslate("Inventory", pPickedItem->GetGameNameEntry()) + _W(" ") +
                                kTranslate("Inventory", "with") + _W(" ") +
                                kTranslate("Inventory", pItem->GetGameNameEntry()) + _W(".");
...
bool cLuxInventory_Slot::OnUIButtonRelease(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (pPickedItem && pItem && pItem == pPickedItem) {
        if (mpInventory->GetItemPickedAt() + 0.5f > gpBase->mpEngine->GetGameTime() &&
            !mpInventory->GetPickedItemMoved()) {
            return OnMouseDoubleClick(apWidget, aData);
        }
    }
...
bool cLuxInventory_Slot::OnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (pItem && pItem != mpInventory->GetPickedItem()) {
        cVector3f vItemPos =
                apWidget->GetGlobalPosition() + cVector3f((vSize - pItem->GetImage()->GetImageSize())) * 0.5f;
        vItemPos.z += 0.1f;
        ...
        if (mfGlowAlpha > 0) {
            ...
            for (int i = 0; i < 2; ++i)
                mpInventory->mpGuiSet->DrawGfx(pItem->GetImage(), vItemPos + cVector3f(0, 0, 0.1f), -1,
                                               cColor(fPulseAlpha * mfGlowAlpha * mpInventory->mfAlpha, 1),
        ...
        if (pType->HasCount()) {
            cVector3f vPos =
                    apWidget->GetGlobalPosition() + cVector3f(vSize.x, 0, 0) + mpInventory->mvLayout_ItemCountOffset;
...
void cLuxInventory::Update(float afTimeStep) {
    ...
    if (!mbMessageActive && mfMessageTextAlpha > 0) // mfMessageTextLife <=0
    {
...
void cLuxInventory::OnDraw(float afFrameTime) {
    ...
    if (mbExitToJournal || mbEnterFromJournal) {
        mpGuiSet->DrawGfx(mpWhiteGfx, mvGuiSetStartPos + cVector3f(0, 0, 20), mvGuiSetSize,
                          cColor(0, 1.0f - mfAlpha * mfAlpha));
    }
    ...
    */
    ...
    if (mfMessageTextAlpha > 0) {
        if (msMessageText != _W("")) {
            ...
            mpGuiSet->DrawGfx(mpWhiteGfx, cVector3f(mvGuiSetStartPos.x, mvLayout_PopUpMessageCenter.y - 20,
                                                    mvLayout_PopUpMessageCenter.z - 0.1f),
                              cVector2f(mvGuiSetSize.x, mvLayout_FontSize_Message.y + 20 * 2),
                              cColor(1.0f - fAlpha * 0.8f), eGuiMaterial_Modulative);
        }
    }
    ...
    if (msHeaderText != _W(""))
        mpGuiSet->DrawFont(msHeaderText, mpFontHeader, vHeaderCenter, mvLayout_FontSize_Header, cColor(1, mfAlpha),
                           eFontAlign_Center);
    ...
    if (mfOldDescTextAlpha > 0) {
        if (msOldHeaderText != _W(""))
            mpGuiSet->DrawFont(msOldHeaderText, mpFontHeader, vHeaderCenter, mvLayout_FontSize_Header,
                               cColor(1, mfAlpha * mfOldDescTextAlpha), eFontAlign_Center);
...
void cLuxInventory::DrawDescTextRow(const tWString &asText, const cColor &aCol) {
    ...
    for (size_t i = 0; i < vRows.size(); ++i) {
        ...
        mpGuiSet->DrawFont(vRows[i], mpFontDefault, cVector3f(vMessageBodyCenter.x - fHalfWidth, fY, 1), vFontSize.y,
                           finalCol, eFontAlign_Left);
    }
}
...
static bool
CheckItemTypeCombo(cLuxInventory_Item *apItemA, cLuxInventory_Item *apItemB, eLuxItemType aTypeA, eLuxItemType aTypeB) {
    return (apItemA->GetType() == aTypeA && apItemB->GetType() == aTypeB) ||
           (apItemA->GetType() == aTypeB && apItemB->GetType() == aTypeA);
}

bool
cLuxInventory::CheckSpecialCombineAction(cLuxInventory_Item *apItemA, cLuxInventory_Item *apItemB, int alSlotIndex) {
    /////////////////////////////
...
void cLuxInventory::CreateGui() {
    ...
    for (int i = 0; i < 4; ++i) {
        mpSanityStatus[i] = pGui->CreateGfxImage("inventory/inventory_sanity_" + sStatusSuffix[i] + ".tga",
                                                 eGuiMaterial_Alpha);
        mpSanityGlow[i] = pGui->CreateGfxImage("inventory/inventory_sanity_glow_" + sStatusSuffix[i] + ".tga",
                                               eGuiMaterial_Alpha);

        mpHealthStatus[i] = pGui->CreateGfxImage("inventory/inventory_health_" + sStatusSuffix[i] + ".tga",
                                                 eGuiMaterial_Alpha);
        mpHealthGlow[i] = pGui->CreateGfxImage("inventory/inventory_health_glow_" + sStatusSuffix[i] + ".tga",
                                               eGuiMaterial_Alpha);
    ...
    for (int i = 0; i < 3; ++i)
        mpOilBubble[i] = pGui->CreateGfxImage("inventory/inventory_oil_bubble_" + cString::ToString(i + 1) + ".tga",
                                              eGuiMaterial_Alpha);
    ...
    pOil->SetFocusNavigation(eUIArrow_Left,
                             mvSlots[1 * mlLayout_SlotsColumns + mlLayout_SlotsColumns - 1]->GetImageWidget());
    pOil->SetFocusNavigation(eUIArrow_Down, pJournal);
    pJournal->SetFocusNavigation(eUIArrow_Up, pOil);
    pJournal->SetFocusNavigation(eUIArrow_Left,
                                 mvSlots[2 * mlLayout_SlotsColumns + mlLayout_SlotsColumns - 1]->GetImageWidget());
    ...
    mvSlots[mlLayout_SlotsColumns + mlLayout_SlotsColumns - 1]->GetImageWidget()->SetFocusNavigation(eUIArrow_Right,
                                                                                                     pOil);
    mvSlots[2 * mlLayout_SlotsColumns + mlLayout_SlotsColumns - 1]->GetImageWidget()->SetFocusNavigation(eUIArrow_Right,
                                                                                                         pOil);
}

//-----------------------------------------------------------------------

void cLuxInventory::DrawElementAtCenter(const cVector3f &avCenter, cGuiGfxElement *apGfx, float afAlpha,
                                        const cVector2f &avSize, bool abMulColWithAlpha) {
...
bool cLuxInventory::OilOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    cGuiClipRegion *pChildClipRegion = pOldClipRegion->CreateChild(
            vOilStart + cVector3f(0, vFullOilSize.y - fRemainingOilHeight, 0.6f),
...
bool cLuxInventory::JournalOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    mpGuiSet->DrawGfx(mpJournalMouseOverGfx, apWidget->GetGlobalPosition() + cVector3f(vSizeAdd.x, vSizeAdd.y, 0.5f),
                      -1, cColor(1, fAlpha));
...
bool cLuxInventory::ItemSlotsOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (mbLayout_DrawSlotsGrid) {
        // Get grid lines size
        cVector2f vHoriSize = cVector2f(
                apWidget->GetSize().x + mvLayout_SlotGridHExtension.x + mvLayout_SlotGridHExtension.y,
                mvLayout_SlotsSeparation.y);
        cVector2f vVertSize = cVector2f(mvLayout_SlotsSeparation.x,
                                        apWidget->GetSize().y + mvLayout_SlotGridVExtension.x +
                                        mvLayout_SlotGridVExtension.y);
        ...
        for (int i = 1; i < mlLayout_SlotsRows; ++i)
            mpGuiSet->DrawGfx(mpWhiteGfx, vPos + cVector3f(-mvLayout_SlotGridHExtension.x, vLineSeparation.y * i, 0),
                              vHoriSize, lineColor);

        for (int i = 1; i < mlLayout_SlotsColumns; ++i)
            mpGuiSet->DrawGfx(mpWhiteGfx, vPos + cVector3f(vLineSeparation.x * i, -mvLayout_SlotGridVExtension.x, 0),
                              vVertSize, lineColor);
    }
...
void cLuxInventory::StatusWidgetUpdate(iWidget *apWidget, int alStatusIdx, cGuiGfxElement **avStatusGfx,
...
void cLuxInventory::DrawFrameAroundWidget(cGuiGfxElement **apGfxCorners, cGuiGfxElement **apGfxBorders,
    ...
    DrawFrame(apGfxCorners, apGfxBorders,
              vPosition - cVector3f(apGfxCorners[0]->GetActiveSize().x + avHPadding.x,
                                    apGfxCorners[0]->GetActiveSize().y + avVPadding.x, 0) + avOffset,
