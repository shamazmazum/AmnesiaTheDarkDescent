...
void LuxCalcGuiSetOffset(const cVector2f &avVirtualSizeIn, const cVector2f &avScreenSize, cVector2f &avOutSize,
...
void LuxCalcGuiSetScreenOffset(const cVector2f &avVirtualSizeIn, cVector2f &avOutSize, cVector2f &avOutOffset) {
    LuxCalcGuiSetOffset(avVirtualSizeIn, gpBase->mpEngine->GetGraphics()->GetLowLevel()->GetScreenSizeFloat(),
                        avOutSize, avOutOffset);
}
...
bool cLuxCustomStorySettings::CreateFromPath(const tWString &asPath) {
    ...
    if (bValid) {
        ...
        msDefaultExtraLanguage = pCustomStoryCfg->GetString("Main", "DefaultExtraLangFile",
                                                            gpBase->msDefaultGameLanguage);
    } else {
...
static unsigned char gv_main_init_str[27] = {0x4B, 0x4A, 0xC1, 0xA5, 0x8, 0x40, 0x3A, 0xA4, 0x5C, 0x4D, 0x5B, 0x5C,
                                             0x77, 0x45, 0x49, 0x41, 0x46, 0x77, 0x41, 0x46, 0x41, 0x5C, 0x6, 0x4B,
                                             0x4E, 0x4F, 0};
static unsigned char gv_error_mess_str[29] = {0x6C, 0x4A, 0x8F, 0xAD, 0xE, 0x53, 0x35, 0xB9, 0x4D, 0x5B, 0x5B, 0x8,
                                              0x5F, 0x41, 0x5C, 0x40, 0x8, 0x5C, 0x40, 0x4D, 0x8, 0x4E, 0x41, 0x44,
                                              0x4D, 0x5B, 0x9, 0x9, 0};
...
static unsigned char gv_start_pos_str[18] = {0x78, 0x49, 0xCE, 0xBA, 0x4, 0x55, 0x46, 0xA0, 0x49, 0x5A, 0x5C, 0x69,
                                             0x5A, 0x4D, 0x49, 0x77, 0x19, 0};
...
bool cLuxBase::ParseCommandLine(const tString &asCommandline) {
    ...
    if (asCommandline == "ptest") {
        ...
        msInitConfigFile = cString::To16Char(
                DecryptString((char *) gv_main_init_str)); //_W("config/ptest_main_init.cfg");
...
bool cLuxBase::InitApp() {
    ...
    msBaseSavePath = sPersonalDir + PERSONAL_RELATIVEROOT
    PERSONAL_RELATIVEGAME_PARENT + msMainSaveFolder + _W("/");
...
bool cLuxBase::CheckFeatureSupport() {
    ...
    // Features
    {
        ...
        if (pLowLevelGfx->GetCaps(eGraphicCaps_PackedDepthStencil) == 0) {
            msErrorMessage = _W(
                    "Packed Depth and Stencil not supported! Make sure your graphic card drivers are up to date!\n");
            return false;
        }
    }
...
cConfigFile *
cLuxBase::LoadConfigFile(const tWString &asDefaultPath, const tWString &asWantedPath, bool abForceLoadDefault,
                         bool *abDidLoadDefault) {
    cConfigFile *pConfig;
...
bool cLuxBase::InitUserConfig() {
    ...
    mpUserKeyConfig = LoadConfigFile(msDefaultUserKeyConfigPath, msMainProfileSavePath + _W("user_keys.cfg"), false,
                                     &bDidLoadDefault);
...
bool cLuxBase::InitEngine() {
    ...
    cRendererDeferred::SetGBufferType(
            (eDeferredGBuffer) mpMainConfig->GetInt("Graphics", "GBufferType", eDeferredGBuffer_32Bit));
...
void cLuxBase::SetProfile(const tWString &asName) {
    ...
    if (msProfileName != _W("")) {
        ...
        Log(" Setting profile: '%s' Path: '%s'\n", cString::To8Char(asName).c_str(),
            cString::To8Char(msMainProfileSavePath).c_str());
    } else {
    ...
    */
    ...
    if (mpCustomStory && mpCustomStory->msExtraLangFilePrefix != "") {
        tString sExtraLangFileName =
                cString::To8Char(mpCustomStory->msStoryRootFolder) + mpCustomStory->msExtraLangFilePrefix +
                sGameFileName;
